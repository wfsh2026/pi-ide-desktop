...
- 2026-05-13：完成会话切换体验修复。点击左侧会话后延迟 250ms 后台预启动对应 Pi 实例，让 UI 先完成切换；新增会话输入队列，用户在 Pi 启动中发送的首条消息会先排队，Pi 启动完成后自动发送，避免首条消息丢失；Pi 环境检测增加 5 分钟缓存；空会话不再因启动欢迎输出触发 `--continue`；会话视图移除最近记录窗口限制，恢复完整 turn 渲染，保证文本和折叠详情完整显示；终端历史 replay 恢复完整写入，默认 scrollback 提升到 100000，并改回 xterm 原生滚动条；Pi IDE bridge 事件文本和工具结果默认上限提升到 10000000，避免会话视图因 extension 事件截断导致文本丢失；终端视图停止过滤 alt-screen/clear-screen 等 ANSI 控制序列，改为把真实 PTY 字节流交给 xterm 解释，避免 TUI 重绘帧被错误追加到 scrollback 导致回复文本错乱和滚动长度异常。
